...
function fetchMediaBatch(folderNames) {
    // Ensure the folder names are valid and HTTPS is used in the URL.
    const foldersParam = encodeURIComponent(folderNames.join(','));
    const url = `https://new-vhym.onrender.com/api/get-cloudinary-media?folders=${foldersParam}`;

    console.log(`Fetching media for folders: ${folderNames.join(', ')}`);

    // Use fetch to retrieve media data
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                // Add any required headers here for security
            },
            credentials: 'same-origin', // Ensures cookies are sent if needed (for sessions, etc.)
        })
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            data.forEach(media => {
                ...
                if (lightbox) {
                    // Check if this is the first image of this folder
                    if (!addedFolders[folderName]) {
                        ...
                        addedFolders[folderName] = true; // Mark this folder as processed (first image added)
                    }
                } else {
