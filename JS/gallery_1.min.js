...
function openLightbox(lightboxId) {
    ...
    // Ensure the id is a number and you are concatenating it correctly
    // Ensure the id is a number and you are concatenating it correctly
    const lightboxId = `Lightbox_${id}`; // Create the full string, e.g., "Lightbox_23"
...
// Function to display media files and manage the slide state
function displayMediaFiles(lightboxId, mediaFiles, folderName) {
    ...
    mediaFiles.forEach((file, index) => {
        ...
        slideDiv.classList.add(`slide`, `slide${lightboxId.replace('Lightbox_', '')}`);
        ...
        image.id = `media-${file.alt.replace(/\s+/g, '-')}`;
...
let slideIndex = parseInt(sessionStorage.getItem(`slideIndex_${lightboxId}`)) || 1;
