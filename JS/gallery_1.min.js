...
function openLightbox(lightboxId) {
    ...
    if (!lastSlide) {
        // Use your new function that handles slide initialization and setting the active slide
        const slides = document.querySelectorAll(`#${lightboxId} .slide`);
        if (slides.length > 0) {
            setActiveSlide(lightboxId, 0); // Start from the first slide (index 0)
        }
    } else {
        // If there's a stored last slide, go to that slide
...
function displayMediaFiles(lightboxId, mediaFiles) {
    ...
    // Retrieve the last known slide index from sessionStorage
    const lastSlideIndex = sessionStorage.getItem(`lastSlideIndex_${lightboxId}`);
    const startingSlideIndex = lastSlideIndex !== null ? parseInt(lastSlideIndex) : 1;

    // Now that all slides are added, apply the active/next/previous classes
    showSlide(lightboxId, startingSlideIndex);
}

// Function to set the active slide and assign previous/next classes
...
// Example of how to trigger when a slide is changed (e.g., via a next/previous button)
