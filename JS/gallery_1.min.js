function fetchMedia(folderId) {
    console.log('Fetching media for folderId:', folderId);

    // Ensure a valid folderId is passed
    if (!folderId) {
    ...
    fetch(`/fetch-media?folderId=${folderId}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(mediaFiles => {
            ...
            if (mediaFiles && mediaFiles.length > 0) {
                // Insert the first media file into the lightbox
                insertFirstMedia(folderId, mediaFiles[0]);
            } else {
                console.warn('No media files found for folderId:', folderId);
            }
        })
        .catch(error => {
...
function initializeLightboxes() {
    ...
    lightboxes.forEach(lightbox => {
        const folderId = lightbox.getAttribute('data-folder-id');
        if (folderId) {
            // Fetch the media when the lightbox is ready
            fetchMedia(folderId);
        } else {
            console.warn('No folderId found for lightbox:', lightbox);
        }
    });
}

document.addEventListener('DOMContentLoaded', () => {
    console.log('Document loaded, initializing lightboxes...');
    initializeLightboxes();
});



// Function to preload initial media for lightboxes
function preloadInitialMedia() {
    const lightboxElements = document.querySelectorAll('.lightbox'); // Assuming your lightboxes are set up like this

    lightboxElements.forEach((lightbox) => {
        const folderId = lightbox.dataset.folderId;

        // Check if media for this folder has already been cached
        if (sessionStorage.getItem(folderId)) {
            console.log(`Media for Lightbox ${folderId} already loaded from cache.`);
            return;
        }

        // If not in cache, fetch and preload media
        fetchMedia(folderId);
    });
}
...
function preloadFirstMedia() {
    ...
    lightboxElements.forEach((lightbox) => {
        const folderId = lightbox.dataset.folderId;
        const firstMediaKey = `firstMedia-${folderId}`;

        // Check if first media for this folder is already cached
        if (sessionStorage.getItem(firstMediaKey)) {
            console.log(`First media for Lightbox ${folderId} already loaded from cache.`);
            const media = JSON.parse(sessionStorage.getItem(firstMediaKey));
            insertFirstMedia(folderId, media[0]); // Directly insert the cached first media
            return;
        }

        // If not in cache, fetch and preload first media
        fetch(`/fetch-media?folderId=${folderId}`)
            .then((response) => response.json())
            .then((mediaFiles) => {
                if (!Array.isArray(mediaFiles)) {
                    console.error(`Invalid response for Lightbox ${folderId}:`, mediaFiles);
                    return;
                }

                if (mediaFiles.length > 0) {
                    const firstMedia = [mediaFiles[0]]; // Only take the first media
                    sessionStorage.setItem(firstMediaKey, JSON.stringify(firstMedia));
                    insertFirstMedia(folderId, firstMedia[0]); // Insert the first media into the DOM
                    console.log(`First media for Lightbox ${folderId} preloaded and inserted.`);
                } else {
                    console.log(`No media found for Lightbox ${folderId}. Skipping preload.`);
                    // Ensure sessionStorage does NOT store empty values
                    sessionStorage.removeItem(firstMediaKey);
                }
            })
            .catch((error) => {
                console.error(`Error fetching media for Lightbox ${folderId}:`, error);
            });
    });
}

function insertFirstMedia(folderId, firstMedia) {
    console.log('Inserting first media for folderId:', folderId, 'Media:', firstMedia);

    // Find the lightbox modal for the given folderId
    const lightbox = document.querySelector(`.lightbox[data-folder-id="${folderId}"]`);
    if (!lightbox) {
        console.error('Lightbox not found for folderId:', folderId);
        return;
    }

    // Insert the first media into the lightbox (assuming it's an image)
    const mediaContainer = lightbox.querySelector('.modal-content'); // Modify this based on your modal structure
    if (!mediaContainer) {
        console.error('Media container not found inside lightbox');
        return;
    }

    const mediaElement = document.createElement('img');
    mediaElement.src = firstMedia.url; // Assuming 'url' is the property for the image URL
    mediaElement.alt = firstMedia.altText || 'Media'; // Optional: set an alt text
    mediaElement.classList.add('lightbox-media'); // Add a class for styling, if necessary

    // Clear previous content and insert the new image
    mediaContainer.innerHTML = ''; // Clear any existing content
    mediaContainer.appendChild(mediaElement);
}

// Call the preload function when the DOM is ready
document.addEventListener("DOMContentLoaded", preloadFirstMedia);

// Fetch media data for the gallery
function fetchMediaData(folderName, lightboxId, isMobile, retries = 3) {
    const url = `https://new-vhym.onrender.com/api/get-cloudinary-media?folder=${folderName}&limit=100`;
    ...
    fetch(url)
        ...
        .then(mediaFiles => {
            // Check if mediaFiles is an array and has content
            if (!Array.isArray(mediaFiles) || mediaFiles.length === 0) {
                console.log("No media files found for this folder.");
                alert("No media files found for the selected folder.");
                return;
            }

            // Continue with the code to display media in the gallery
            console.log("Fetched media files:", mediaFiles);

            // Update gallery here with mediaFiles
            // Example: renderMediaToGallery(mediaFiles);
        ...
        .catch(err => {
            ...
            if (retries > 0) {
                ...
                alert(`Failed to fetch media for Lightbox ${lightboxId}: ${err.message}`);
            }
        });
}
...
// Call the preload function when the DOM is ready
document.addEventListener("DOMContentLoaded", preloadInitialMedia);
