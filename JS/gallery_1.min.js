...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize the lightboxes after the DOM is ready
    initializeLightboxes();

    // Preload the initial media for the lightboxes
    preloadInitialMedia();

    // Preload first media for the lightboxes (optional)
    preloadFirstMedia();
});
...
function initializeLightboxes() {
    ...
    console.log('Found lightboxes:', lightboxes);

    // Loop through each lightbox
    lightboxes.forEach((lightbox, index) => {
        const folderId = `folder_${index + 1}`; // Use index to generate folderId
        console.log(`Assigning folderId "${folderId}" to lightbox_${index + 1}`);

        // Assign folderId to lightbox's data-folder-id attribute
        lightbox.setAttribute('data-folder-id', folderId);

        // Fetch media for each lightbox
        fetchMedia(folderId);
    });
}
...
function fetchMedia(folderId) {
    ...
    // Make a fetch request to get the media for the specified folderId
    fetch(`/fetch-media?folderId=${folderId}`)
        ...
        .then(mediaFiles => {
            ...
            // If no media files exist, skip the lightbox
            if (!mediaFiles || mediaFiles.length === 0) {
                console.log(`No media found for folderId: ${folderId}, skipping...`);
                return; // Skip this lightbox if it has no media
            }

            // Insert the first media into the lightbox if it has data
