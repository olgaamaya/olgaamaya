...
// Function to insert first media into the correct Lightbox structure
function insertFirstMedia(folderId, media) {
    const lightbox = document.getElementById(`Lightbox_${folderId}`);
    if (!lightbox) {
        console.error(`Lightbox_${folderId} not found in the DOM.`);
        return;
    }

    const modalContent = lightbox.querySelector('.modal-content');
    if (!modalContent) {
        console.error(`.modal-content not found in Lightbox_${folderId}`);
        return;
    }

    const slideDiv = document.createElement('div');
    slideDiv.classList.add('slide', `slide${folderId}`);
    ...
    if (media.type === "video") {
        ...
        img.loading = 'eager';
        img.alt = `Lightbox ${folderId} Media`;
        slideDiv.appendChild(img);
    }

    modalContent.appendChild(slideDiv);
    console.log(`Inserted first media for Lightbox_${folderId}`);
}

// Call function to preload the first media when the page loads
