...
// Function to fetch media data and handle caching
function fetchMedia(folderId, retries = 3) {
    fetch(`https://new-vhym.onrender.com/api/get-cloudinary-media?folder=${folderId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`Error fetching media for Lightbox ${folderId}: ${response.statusText}`);
            }
            return response.json();
        })
        .then(data => {
            // Cache the media in sessionStorage
            sessionStorage.setItem(folderId, JSON.stringify(data));
            console.log(`Media for Lightbox ${folderId} loaded successfully.`);
        })
        .catch(error => {
            console.error(error);
            if (retries > 0) {
                console.log(`Retrying... Remaining retries: ${retries}`);
                setTimeout(() => fetchMedia(folderId, retries - 1), 2000); // Retry logic with a delay
            } else {
                alert(`Error preloading media for Lightbox ${folderId}: ${error.message}`);
            }
        });
}
