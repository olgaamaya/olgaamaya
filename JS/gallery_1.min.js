...
function preloadFirstMedia() {
    ...
    lightboxElements.forEach((lightbox) => {
        const folderId = lightbox.dataset.folderId;
        const firstMediaKey = `firstMedia_${folderId}`;

        console.log(`Checking cache for first media of Lightbox ${folderId}`);

        // Check if first media is already cached
        if (sessionStorage.getItem(firstMediaKey)) {
            console.log(`First media for Lightbox ${folderId} already loaded from cache.`);
            return;
        }

        console.log(`Fetching first media for Lightbox ${folderId}`);

        // Fetch only the first media
        fetch(`https://new-vhym.onrender.com/api/get-cloudinary-media?folder=${folderId}`)
            .then(response => response.json())
            .then(mediaFiles => {
                console.log("Fetched mediaFiles:", mediaFiles); // Log the response to inspect its structure

                // Check if mediaFiles contains an error
                if (mediaFiles.error) {
                    console.log(`Error: ${mediaFiles.error}. Skipping preload.`);
                    return; // Exit early if no media found or error message is returned
                }

                // Ensure mediaFiles is an array and contains data
                if (Array.isArray(mediaFiles) && mediaFiles.length > 0) {
                    ...
                    console.log(`First media for Lightbox ${folderId} preloaded.`);
                } else {
                    console.log(`No media found for Lightbox ${folderId}. Skipping preload.`);
                }
            })
            .catch(error => {
                console.error(`Error preloading first media for Lightbox ${folderId}:`, error);
            });
    });
}
...
function fetchMediaData(folderName, lightboxId, isMobile, retries = 3) {
    fetch(`https://new-vhym.onrender.com/api/get-cloudinary-media?folder=${folderName}`)
        ...
        .then(mediaFiles => {
            ...
            // Check if mediaFiles contains an error
            if (mediaFiles.error) {
                console.log(`Error: ${mediaFiles.error}. Skipping media fetch.`);
                return; // Exit early if no media found or error message is returned
            }

            // Ensure mediaFiles is an array before processing
            if (!Array.isArray(mediaFiles)) {
                console.error(`Error: mediaFiles is not an array. Response is:`, mediaFiles);
                return; // Exit early if mediaFiles is not an array
            }

            // Check if mediaFiles has data
            if (mediaFiles.length === 0) {
                console.log(`No media found for Lightbox ${lightboxId}. Skipping media fetch.`);
                return;
            }
...
function displayMediaFiles(mediaFiles, lightboxId, isMobile) {
    const lightbox = document.getElementById(`Lightbox_${lightboxId}`);
    const mediaContainer = lightbox.querySelector(".modal-content");
    const pageNumberContainer = lightbox.querySelector(".page-number"); // Element to display page number

    // Ensure existing content is not removed (clear only if required)
    mediaContainer.innerHTML = ''; // This line is optional based on your preference

    // Set the total media count
    const totalPages = mediaFiles.length;

    // Loop through each media file and append it to the lightbox
    mediaFiles.forEach((media, index) => {
        ...
        // Display the current media number and total media count (page number)
        const currentPageNumber = index + 1; // current media number (1-based index)

        // Update the page number display
        pageNumberContainer.textContent = `${currentPageNumber} of ${totalPages}`;
    });

    // Initialize the first slide (always start at the first media item)
    showSlide(lightboxId, 1); // Ensure the first media item is shown initially

    // Adjust styling for media depending on mobile or desktop
    if (isMobile) {
        mediaContainer.style.maxWidth = "100%";
        mediaContainer.style.maxHeight = "90vh";
    } else {
        mediaContainer.style.maxWidth = "80%";
        mediaContainer.style.maxHeight = "80vh";
    }

    // Ensure the first slide is displayed correctly
    console.log(`Media files displayed for Lightbox ${lightboxId}`);
}
