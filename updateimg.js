...
async function fetchImagesFromFolder(folderName) {
    try {
        ...
        return response.data; // Assuming this returns a list of image URLs
    } catch (error) {
...
async function updateImagesWithAltText() {
    ...
    for (const folderName of folderNames) {
        ...
        // Fetch all image URLs from the current folder using the custom API
        const imageUrls = await fetchImagesFromFolder(folderName);
